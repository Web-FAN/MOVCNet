...
from sympy import Predicate, sequence
...
class MOVCNet(BertPreTrainedModel):
    def __init__(self, 
                 config, 
                 num_labels=102,
                 latent_size=0, 
        ...
        super(MOVCNet, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size * 2, num_labels, bias=False)
        self.apply(self.init_bert_weights)
        self.criterion = nn.MultiLabelSoftMarginLoss()
        self.mse = nn.MSELoss()
        
        # Attention
        self.attention = nn.MultiheadAttention(embed_dim=config.hidden_size, num_heads=4)
        self.obj_to_bert = nn.Linear(2048, config.hidden_size, bias=False)
        ...
        if svd:
            ...
            self.latent_size = int(math.sqrt(num_labels))
        else:
            self.latent_size = latent_size
        self.latent_scalar = nn.Parameter(torch.FloatTensor([.1]))
        self.feat_to_latent = nn.Linear(config.hidden_size * 2, self.latent_size, bias=False)
        self.latent_to_label = nn.Linear(self.latent_size, num_labels, bias=False)
    ...
    def load_svd(self, path):
        ...
        return torch.matmul(mat_u, mat_s)
    
    def forward_nn(self, input_ids, segment_ids, input_mask, obj_repr):
        ...
        cls_repr = cls_repr.squeeze(1)
        cls_repr_copy = cls_repr
        
        cls_repr_copy = cls_repr_copy.unsqueeze(1)  
        obj_repr2 = self.obj_to_bert(obj_repr)
        attn_output, attn_output_weights = self.attention(cls_repr_copy.transpose(0,1), obj_repr2.transpose(0,1), obj_repr2.transpose(0,1))
        
        text_aware_object_repr = attn_output.squeeze(0)
        final_repr = torch.cat([cls_repr, text_aware_object_repr], dim=1)
        outputs = self.classifier(final_repr)
        
        outputs_latent = None
        latent_label = self.feat_to_latent(final_repr)
    ...
    def forward(self, input_ids, segment_ids, input_mask, obj_repr, labels=None):
        outputs, outputs_latent = self.forward_nn(input_ids, segment_ids, input_mask, obj_repr)
    ...
    def predict(self, input_ids, segment_ids, input_mask, obj_repr, predict_top=True):
        self.eval()
        outputs, _ = self.forward_nn(input_ids, segment_ids, input_mask, obj_repr)
